...
namespace TMDbLibTests.Core2
{
    public class ClientMovieTests : TestBase
    {
        ...
        public void TestMoviesExtrasExclusive()
        {
            ...
            IgnoreMissingJson(" / external_ids", "external_ids / id", "recommendations.results[array] / media_type", "recommendations.results[array] / popularity");

            IgnoreMissingCSharp("alternative_titles.titles[array].type / type", "lists.results[array].list_type / list_type", "production_companies[array].logo_path / logo_path", "production_companies[array].origin_country / origin_country", "translations.translations[array].data / data");
        ...
        public void TestMoviesImdbExtrasAll()
        {
            ...
            IgnoreMissingJson(" / account_states", "external_ids / id");
            IgnoreMissingCSharp("alternative_titles.titles[array].type / type", "lists.results[array].list_type / list_type", "production_companies[array].logo_path / logo_path", "production_companies[array].origin_country / origin_country", "translations.translations[array].data / data");
        ...
        public void TestMoviesExtrasAll()
        {
            // We ignore the 'notes' field, as TMDb sometimes leaves it out
            IgnoreMissingJson("release_dates.results[array].release_dates[array] / note");
            IgnoreMissingJson("similar.results[array] / media_type", "external_ids / id", "recommendations.results[array] / media_type", "recommendations.results[array] / popularity");
        ...
        {
            IgnoreMissingCSharp("titles[array].type / type");
        ...
        public void TestMoviesGetMovieCasts()
        {
            ...
            Assert.Equal("5adf0934c3a3686adc009582", crew.CreditId);
        ...
        {
            ResultContainer<Video> resp = Config.Client.GetMovieVideosAsync(IdHelper.TheDarkKnightRises).Result;
            Assert.NotNull(resp);

            Assert.NotNull(resp);
            Assert.NotNull(resp.Results);

            Video video = resp.Results.FirstOrDefault();
            Assert.NotNull(video);

            Assert.Equal("58ba56c4c3a368668f01532c", video.Id);
            Assert.Equal("en", video.Iso_639_1);
            Assert.Equal("US", video.Iso_3166_1);
            Assert.Equal("7gFwvozMHR4", video.Key);
            Assert.Equal("The Dark Knight Rises - Official Trailer #2 [HD]", video.Name);
            Assert.Equal("YouTube", video.Site);
            Assert.Equal(1080, video.Size);
        ...
        {
            IgnoreMissingCSharp("translations[array].data / data");
        ...
        {
            IgnoreMissingCSharp("results[array].list_type / list_type");
